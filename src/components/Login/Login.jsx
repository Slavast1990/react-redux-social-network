...
const LoginForm = ({ handleSubmit, error, captchaUrl }) => { //деструктуризация чтоб не писать props.handleSubmit...
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            {captchaUrl && <img src={captchaUrl} />//если через state придет капча то мы покажем картинку с капчей
            }
            {captchaUrl &&   createField("symbols from image", "captcha", [required], Input, {}) /* "remember me" это текст*/}
...
const Login = (props) => {
    const onSubmit = (FormData) => {
        props.login(FormData.email, FormData.password, FormData.rememberMe, FormData.captcha)//данные из login заходят в FormData и потом с помощью props раскукоживаем их
    }
